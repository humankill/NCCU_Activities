...
public class KeywordList {
	...
	public void add(Keyword keyword){
		...
		for(int i=0; i<lst.size();i++){
			Keyword k= lst.get(i);	{//如果count比原本的小就要放前面,如果count相等就要比weight
...
//		printKeywordList(lst);
	...
	public void outputIndex(int i){ //O
		if(i>lst.size()){
	...
	public void outputCount(int c){//O
	...
	public void outputHas(String pattern){//O
		LinkedList<Keyword> results = new LinkedList<>();
		for(int i=0;i<lst.size();i++){
		    Keyword k = lst.get(i);
	...
	public void outputName(String pattern){//O
	...
	public void outputFirstN(int n){//O
...
public void deleteIndex(int i){
	...
	public void deleteName(String name){
		...
		for(int i=0;i<lst.size();i++){
			Keyword k =lst.get(i);
			if(k.name.contains(name)) found.add(k);
		}
